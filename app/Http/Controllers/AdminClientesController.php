    ...
    */
    ...
    public function registrarClientePost(Request $request){
        ...
        try {
            ...
            $crmuser->id_cms_privileges = 4;
            $crmuser->created_at = date('Y-m-d H:i:s');
            $crmuser->token = Str::random(35);
            $crmuser->save();

            $data= Array(
                "token" => $crmuser['token'],
                "name" => $crmuser['nombres'],
                "email" => $request['correo'],
                "password" => $request['password']
            );
            
// registro del correo notificacion
    ...
    public function activarUsuario($token){
        $user=CmsUser::where('token',$token)->first();
        if (!empty($user)){
            $user->status='Active';
            $user->save();
            return redirect('https://supermercadodoncando.com/');
        }else{
